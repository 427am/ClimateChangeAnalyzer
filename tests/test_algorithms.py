...
class TestAlgorithms(unittest.TestCase):
    ...
    def test_linear_model(self):
        ...
        self.assertIsNotNone(model.slope, "Slope should not be 0.")
        self.assertIsNotNone(model.intercept, "Model should contain an intercept.")

        # Create the predictions array
        y_pred = model.predict(X_test)

        # Assert the number of predictions is equal to the testing set
        self.assertEqual(len(y_pred), len(X_test), "# of predictions must be equal to # of inputs.")
    ...
    def test_sarima_model(self):
        ...
        self.assertEqual(len(future_predictions), steps, "Mismatch in count of predictions.")
        self.assertIsInstance(future_predictions, np.ndarray, "Predictions should be a numpy array.")
    ...
    def test_custom_clustering(self):
        ...
        self.assertEqual(len(np.unique(labels)), 2, "Clustering should produce 2 clusters.")
        self.assertEqual(len(labels), len(data), "Labels should match the data size.")
    ...
    def test_anomaly_detection(self):
        ...
        self.assertEqual(anomalies[0], 4, "Anomaly detection did not detect the correct outlier.")
